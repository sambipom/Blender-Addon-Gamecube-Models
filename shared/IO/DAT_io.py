...
class DATBuilder(BinaryWriter):
    ...
    def __init__(self, path, root_nodes):
        super().__init__(path)
        # leave some padding bytes to be overwritten with the header at the end
        self.seek(DATBuilder.DAT_header_length)
    ...
    def build(self):
        ...
        for node in self.node_list:
            ...
            alignment = get_alignment_at_offset(
                first_field[1], self._currentRelativeAddress()
            )
            for i in range(alignment):
                _ = self.write(0, 'uchar')

            node.address = (
                self._currentRelativeAddress() + node.allocationOffset()
            )
    ...
    def write(self, value, field_type, address=None, relative_to_header=True, whence='start'):
        ...
        if isBracketedType(field_type):
            return self.write(
                value, getBracketedSubType(field_type), address, relative_to_header, whence
            )

        elif is_primitive_type(field_type):
            write_address = (
                self._currentRelativeAddress() if relative_to_header else self.currentAddress()
            )
